...
export function verifyJWT(protection: "admin" | "user") {
  //TODO clean up toResponse() repeated code
  //?? make toResponse() middleware
  return (req: Request, res: Response, next: NextFunction) => {
    //return unauthorized if auth cookie not provided
    if(!req.cookies.auth) { 
      return res.status(401).send(toResponse("unauthorized", true))
    }
...
export function verifyWebhook() {
  ...
  return ((req: Request, res: Response, next: NextFunction) => {
    ...
    if(!signature) { return res.status(401).send(toResponse("unauthorized")) }
    ...
    try {
      ...
      //auth failed
      res.status(401).send(toResponse("unauthorized"));
    }
  })
}